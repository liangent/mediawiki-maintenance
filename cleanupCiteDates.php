...
class CleanupCiteDates extends PageDomMaintenanceExt {
	...
	public function cleanupDateString( $date ) {
		...
		$date = mb_convert_kana( $date, 'rnas', 'utf-8' ); // full-width to half-width
		...
		} elseif ( preg_match( '/^(\d{4})\s*[–\-\/\.]\s*(\d{1,2})\s*[–\-\/\.]\s*(\d{1,2})(?:[T\s]+\d+\s*:\s*\d+(?:\s*:\s*\d+)?(?:\s*\+\s*\d+(?:\s*:\s*\d+)?|\s*Z)?)?$/u', $date, $matches ) ) {
			list( $_, $year, $month, $day ) = array_map( 'intval', $matches );
